...
export default function Hotels() {
  ...
  const displayRows = useMemo(() => {
    return filtered.map(r => ({
      ...r,
      price_from: formatInt(r.price_from),
      rating_avg: formatRating(r.rating_avg),
      offers_count: formatInt(r.offers_count),
    }))
  }, [filtered])
  ...
  const resortsForCountry = (meta?.resorts || []).filter(
    r => String(r.country_id) === String(form.country_id)
  )
    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <input
          ...
          placeholder="Поиск по отелю/стране/курорту..."
      ...
      <DataTable columns={columns} rows={displayRows} rowKey="id" onRowClick={edit} />
        ...
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
          <Field label="country_id">
            <select
              ...
              onChange={(e) => setForm({ ...form, country_id: e.target.value, resort_id: '' })}
            >
              <option value="">— выбрать —</option>
              {(meta?.countries || []).map(c => (
                <option key={c.id} value={c.id}>
                  {c.name_ru} ({c.iso_code})
                </option>
              ))}
            </select>
          </Field>
